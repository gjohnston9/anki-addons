...
re_brackets_parentheses = re.compile(r"\[[^\]]*\]|\([^\)]*\)")
...
def keyHandler(self, evt, _old):
	...
	if key == TEST_KEY:
		...
		# also remove translations in parentheses from example sentence
		# (like "虹(にじ: rainbow) ...")
		answer = re.sub(re_brackets_parentheses, "", answer)

		if key == ANSWER_SEARCH_KEY:
			encoded = answer.encode('utf8', 'ignore')
			search = SEARCH_URL
		else:
			encoded = answer[:-1].encode('utf8', 'ignore') # remove last character of answer (the period) so that the sentence will be found on Jisho
